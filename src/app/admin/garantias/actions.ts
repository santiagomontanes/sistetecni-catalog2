...
export async function getAfterSalesOrigin(payload:{accessToken:unknown;saleItemId:unknown}):Promise<AdminResult<AdminAfterSalesOriginDTO>> {
  return withAdmin(payload.accessToken,async(client)=>{
    ...
    const [{data:sale,error:saleError},{data:unit,error:unitError},{count,error:caseError}]=await Promise.all([
      ...
      client.from("after_sales_cases").select("id",{count:"exact",head:true}).eq("product_unit_id",item.product_unit_id).not("status","in",'("closed","cancelled")')
    ]);
    ...
    const warrantyExpiresAt=sale.warranty_months>0?new Date(new Date(sale.created_at).setMonth(new Date(sale.created_at).getMonth()+sale.warranty_months)).toISOString():null;
...
export async function openAfterSalesCase(payload:{accessToken:unknown;[key:string]:unknown}):Promise<AdminResult<AdminAfterSalesCaseDetailDTO>> {
  const {accessToken,...input}=payload; return withAdmin(accessToken,async(client)=>{
    ...
    const repo=createAfterSalesCasesRepository(client); const id=await repo.open(parsed.data); const detail=await repo.findById(id); if(!detail) return {ok:false,error:"NOT_FOUND"}; return {ok:true,data:detail};
  });
}
...
export async function progressAfterSalesCase(payload:{accessToken:unknown;[key:string]:unknown}):Promise<AdminResult<AdminAfterSalesCaseDetailDTO>> {
  const {accessToken,...input}=payload; return withAdmin(accessToken,async(client)=>{
    ...
    const repo=createAfterSalesCasesRepository(client); const id=await repo.progress(parsed.data); const detail=await repo.findById(id); if(!detail) return {ok:false,error:"NOT_FOUND"}; return {ok:true,data:detail};
  });
}
