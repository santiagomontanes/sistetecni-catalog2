...
export default function GarantiasPage(){
  const [items,setItems]=useState<AdminAfterSalesCaseListItemDTO[]>([]); const [status,setStatus]=useState<"all"|AfterSalesCaseStatus>("all"); const [query,setQuery]=useState(""); const [loading,setLoading]=useState(true); const [error,setError]=useState("");
  ...
  return <div className="space-y-6">
    ...
    <form onSubmit={(e)=>{e.preventDefault();void load();}} className="flex flex-wrap gap-2"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="GAR-, DEV-, cliente, documento, celular, STU o serial" className="min-w-[260px] flex-1 rounded-xl border border-border px-4 py-3 text-sm"/><button className="rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-white">Buscar</button></form>
