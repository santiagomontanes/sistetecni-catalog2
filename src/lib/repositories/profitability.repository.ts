...
export function createProfitabilityRepository(client:SupabaseClient):ProfitabilityRepository{return{
  async listDashboard(limit=100){
    ...
    const result:AdminProfitabilitySaleDTO[]=saleRows.map(s=>{
      ...
      const unitSet=new Set(physical.map(i=>i.product_unit_id!));const unitExtras=sumCosts(costs.filter(c=>c.product_unit_id&&unitSet.has(c.product_unit_id)));const saleCosts=sumCosts(costs.filter(c=>c.sale_id===s.id));const known=acquisition+unitExtras+saleCosts;const revenue=n(s.total_cop);const profit=revenue-known;
      ...
      return{saleId:s.id,saleNumber:s.sale_number,createdAt:s.created_at,customerName:s.customer_name,revenueCop:revenue,acquisitionCostCop:acquisition,unitExtraCostsCop:unitExtras,saleCostsCop:saleCosts,knownCostCop:known,knownProfitCop:profit,marginPercent:safeMarginPercent(profit,revenue),physicalItemCount:physical.length,manualItemCount:manual.length,costingStatus,units:refs};
    });
